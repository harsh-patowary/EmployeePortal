...
function LeaveDashboardPage() {
    ...
    useEffect(() => {
        if (actionLoading === 'succeeded' || actionLoading === 'failed') {
            ...
            if (actionLoading === 'succeeded') {
                ...
                const errorMessage = typeof actionError === 'string' ? actionError :
                                     actionError?.detail || actionError?.reason || 'An error occurred.';
    ...
    const requestsForApproval = canApproveHr ? hrPending : (canApproveManager ? managerPending : []);
    ...
    return (
        <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
            ...
            {loading === 'pending' && <CircularProgress sx={{ display: 'block', margin: 'auto' }} />}
            {error && <Alert severity="error" sx={{ mb: 2 }}>{typeof error === 'string' ? error : 'Failed to load leave data.'}</Alert>}
